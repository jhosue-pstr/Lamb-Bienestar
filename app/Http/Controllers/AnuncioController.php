...
class AnuncioController extends Controller
{
    ...
    public function masInformacion2($id)
    {
        // Obtener el anuncio específico por ID
        $anuncio = Anuncio::find($id);

        // Verificar si el anuncio existe
        if (!$anuncio) {
            return redirect()->route('anuncios.index')->withErrors('El anuncio solicitado no existe.');
        }

        // Obtener todos los anuncios para el selector
        $anuncios = anuncio::all();

        // Pasar ambas variables a la vista
        return view('mas-informacion2', compact('anuncio', 'anuncios'));
    }
